...
- Topic-owned references: `<topic>/references/`
- Bulky topic-owned raw intake: `<topic>/resources/` with targeted `.gitignore`
  rules
- Broad study-list raw snapshots: `roadmaps/study-list-source-raw/`
- Coding interview imports: `Coding Interview/`
- React TypeScript copied repo: `frontend/react/references/react-typescript-cheatsheet/`

Workflow:

1. Keep copied third-party material under the nearest topic-local `references/`
...
3. Do not mix full copied repos directly into curated folders such as
   `frontend/react/`; use a topic-local `references/` folder.
...
6. Keep large personal screenshot dumps under `roadmaps/study-list-screenshots/`
   or the closest topic-local `resources/` folder; those paths are intentionally
   ignored by Git.
7. Keep broad PDFs, EPUBs, screenshots, and images in ignored raw intake folders
   unless they have been reduced to curated notes.
